...
前提:
    - macOS 26 以降
    - apple_translate がビルド済み
      cd apple_translate && swift build -c release

使い方:
    python translate.py input.csv -o output.csv -c column1 column2
"""
...
SWIFT_CLI = os.path.join(SCRIPT_DIR, "apple_translate", ".build", "release", "apple-translate")
BATCH_SIZE = 200
...
def check_cli():
    if not os.path.isfile(SWIFT_CLI):
        ...
        print("  cd apple_translate && swift build -c release", file=sys.stderr)
        sys.exit(1)
