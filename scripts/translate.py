...
前提:
    - macOS 26 以降
    - apple-translate がビルド済み (swift build -c release) または brew install 済み
...
# brew install 済みなら PATH から、なければローカルビルドを使う
_LOCAL_CLI = os.path.join(PROJECT_ROOT, ".build", "release", "apple-translate")
import shutil as _shutil
SWIFT_CLI = _shutil.which("apple-translate") or _LOCAL_CLI
BATCH_SIZE = 200
...
def check_cli():
    if not os.path.isfile(SWIFT_CLI):
        ...
        print("  または: brew install apple-translate", file=sys.stderr)
        sys.exit(1)
